...
- **사이트 컨셉: 가벼운 입문 게이트웨이** (2026-07-21 확정). 상세 매뉴얼 경쟁(예: koreabybike의
  구간별 페이지·예매 절차 등)은 하지 않는다 — 깊이는 Resources 링크아웃으로 넘기고(가이드 하단
  "Go deeper" 박스), 우리 콘텐츠는 **흥미 유발**(가고 싶게 만들기)을 최우선 기준으로 고른다.
  예: Day One 내러티브, 음식 버킷리스트, by-the-numbers. 새 콘텐츠 제안 시 이 기준으로 거를 것
